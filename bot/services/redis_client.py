...
HISTORY_MAX = HISTORY_PAIRS * 2  # каждая пара = 2 элемента (user + assistant)
...
class RedisClient:
    ...
    async def append_message(self, user_id: int, role: str, content: str) -> None:
        ...
        # Оставляем только последние HISTORY_MAX сообщений
    ...
    async def increment_requests(self, user_id: int) -> int:
        ...
        if count == 1:
            # Первый запрос сегодня — устанавливаем TTL до полуночи UTC
